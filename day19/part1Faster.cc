...
int evaluate(const Blueprint& blueprint, const Robots& robots, const Resources& resources, const int minutesLeft) {
  ...
  if (robots.building()) {
    ...
    if (costFeasible(resources, blueprint.oreRobotCost)) {
      scores.push_back(
          evaluate(blueprint, robots.buildOreRobot(), applyCost(resources, blueprint.oreRobotCost), minutesLeft));
    }
    if (costFeasible(resources, blueprint.clayRobotCost)) {
      scores.push_back(
          evaluate(blueprint, robots.buildClayRobot(), applyCost(resources, blueprint.clayRobotCost), minutesLeft));
    }
    if (costFeasible(resources, blueprint.obsidianRobotCost)) {
      scores.push_back(evaluate(blueprint, robots.buildObsidianRobot(),
                                applyCost(resources, blueprint.obsidianRobotCost), minutesLeft));
    }
    if (costFeasible(resources, blueprint.geodeRobotCost)) {
      scores.push_back(
          evaluate(blueprint, robots.buildGeodeRobot(), applyCost(resources, blueprint.geodeRobotCost), minutesLeft));
    }
    if (scores.size() == 0) {
      scores.push_back(evaluate(blueprint, robots, incrementResources(resources, robots, 1), minutesLeft - 1));
    }
  }
...
int main(int argc, char** argv) {
  ...
  for (const auto& blueprint : blueprints) {
    ...
    auto geodes = evaluate(blueprint, robots, resources, 24);
