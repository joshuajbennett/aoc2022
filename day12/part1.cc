...
int main(int argc, char** argv) {
  ...
  auto startId = map.toId(map.start).value();
  ...
  visited[map.start.y][map.start.x] = VisitInfo{startId, 0};
  ...
  if (!visited[map.end.y][map.end.x].has_value()) {
    throw std::runtime_error("Didn't explore to an end location.");
  }
  std::cout << "Distance to the end: " << visited[map.end.y][map.end.x].value().numSteps << std::endl;
