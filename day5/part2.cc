...
// ```
// cat ../day5/in.txt | day5/day5part2
// ```
...
int main(int argc, char** argv) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    int numCrates = std::stoi(commands[1]);
    int from = std::stoi(commands[3]) - 1;
    int fromSize = crateStacks[from].size();
    if (fromSize < numCrates) {
      throw std::runtime_error("Moving too many crates from a stack.");
    }
    int to = std::stoi(commands[5]) - 1;
    for (int i = 0; i < numCrates; ++i) {
      crateStacks[to].push_back(crateStacks[from][fromSize - numCrates + i]);
    }
    crateStacks[from].resize(fromSize - numCrates);
    // renderCrates(crateStacks);
  }
  renderCrates(crateStacks);
