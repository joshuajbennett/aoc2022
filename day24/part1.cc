...
struct NextNode {
  ...
  bool operator<(const NextNode& rhs) const {
    int heuristicScale = 1;
    return timeTraveled + heuristicScale * distFromDestination >
           rhs.timeTraveled + heuristicScale * rhs.distFromDestination;
  }
};
...
int main(int argc, char** argv) {
  ...
  toVisit.push(NextNode{initialLoc, 0, distFromFinal});
  ...
  while (!toVisit.empty()) {
    ...
    // numIter++;
    // if (numIter % 1000000 == 0) {
    //   std::cout << "Score: " << visiting.timeTraveled + visiting.distFromDestination << std::endl;
    // }
    ...
    if (visiting.location == initialLoc) {
      ...
      for (const auto& neighbor : currentGrid[visiting.location.y][visiting.location.x].neighbors) {
        if (neighbor == finalLoc) {
          std::cout << "Time: " << visiting.timeTraveled + 1 << std::endl;
          return 0;
        } else {
          if (visiting.timeTraveled < maxNumPropagations - 2) {
            ...
            if (visiting.timeTraveled + 1 + distFromFinal < nextGrid[neighbor.y][neighbor.x].gScore) {
              toVisit.push(NextNode{neighbor, visiting.timeTraveled + 1, distFromFinal});
              nextGrid[neighbor.y][neighbor.x].gScore = visiting.timeTraveled + 1 + distFromFinal;
            }
          }
        }
      }
