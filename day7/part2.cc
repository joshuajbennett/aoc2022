...
int findSmallestDirectory(const Directory& dir, const int currentSmallestSize, const int minimumAmount) {
  int smallestSize = currentSmallestSize;
  if (dir.totalSize < smallestSize && dir.totalSize > minimumAmount) {
    smallestSize = dir.totalSize;
  }
  for (const auto& directory : dir.directories) {
    auto smallestInThisDirectory = findSmallestDirectory(directory, smallestSize, minimumAmount);
    if (smallestInThisDirectory < smallestSize && smallestInThisDirectory > minimumAmount) {
      smallestSize = smallestInThisDirectory;
    }
  }
  return smallestSize;
}
...
int main(int argc, char** argv) {
  ...
  std::cout << "Total size:" << root.totalSize << std::endl;

  int sizeToDelete = root.totalSize - (70000000 - 30000000);
  if (sizeToDelete < 0) {
    std::cerr << "Already small enough size." << std::endl;
    return -1;
  }
  std::cout << "Need to delete " << sizeToDelete << std::endl;
  auto smallestDirectorySize = findSmallestDirectory(root, std::numeric_limits<int>::max(), sizeToDelete);
  std::cout << "Smallest directory size to delete: " << smallestDirectorySize << std::endl;
  return 0;
}
